...
#define GET_CANARY(memBlock, memBlockSize, side) \
  getCanary(memBlock, memBlockSize, side)
#define SET_CANARY(memBlock, memBlockSize, canary, side) \
  setCanary(memBlock, memBlockSize, canary, side)
#define SET_CANARIES(memBlock, memBlockSize, canaryL, canaryR) \
  setCanaries(memBlock, memBlockSize, canaryL, canaryR)
#define LIST_CHECK_CANARIES(list) listCheckCanaries(list)
#define LIST_SET_CANARIES(list)                                       \
  SET_CANARIES((void*)list->nodes, list->capacity * sizeof(ListNode), \
               LIST_ARRAY_CANARY_L, LIST_ARRAY_CANARY_R);
...
//!          memBlock.
//-----------------------------------------------------------------------------
void setCanary(void* memBlock, size_t memBlockSize, uint32_t canary,
               char side) {
  if (side == 'l') {
...
//!          memBlock.
//-----------------------------------------------------------------------------
void setCanaries(void* memBlock, size_t memBlockSize, uint32_t canaryL,
                 uint32_t canaryR) {
...
bool listCheckCanaries(List* list) {
  if (getCanary((void*)list->nodes, list->capacity * sizeof(ListNode), 'l') !=
          LIST_ARRAY_CANARY_L ||
...
#endif
{
  ...
  list->size = 0;
  list->capacity = capacity + 1 > LIST_MINIMAL_CAPACITY ? capacity + 1
                                                        : LIST_MINIMAL_CAPACITY;
...
//! @return list if constructed successfully or NULL otherwise.
//-----------------------------------------------------------------------------
List* newList() { return newList(LIST_DEFAULT_CAPACITY); }
...
#define TO_STR(value) #value
  ...
  switch (error) {
    case LIST_NO_ERROR:
      return TO_STR(LIST_NO_ERROR);
    case LIST_POP_FROM_EMPTY:
      return TO_STR(LIST_POP_FROM_EMPTY);
    case LIST_TOP_FROM_EMPTY:
      return TO_STR(LIST_TOP_FROM_EMPTY);
    case LIST_CONSTRUCTION_FAILED:
      return TO_STR(LIST_CONSTRUCTION_FAILED);
    case LIST_REALLOCATION_FAILED:
      return TO_STR(LIST_REALLOCATION_FAILED);
    case LIST_MEMORY_CORRUPTION:
      return TO_STR(LIST_MEMORY_CORRUPTION);
    case LIST_FREE_LIST_LOOP:
      return TO_STR(LIST_FREE_LIST_LOOP);
    case LIST_LOOP:
      return TO_STR(LIST_LOOP);
    case LIST_ACCESSING_ZERO:
      return TO_STR(LIST_ACCESSING_ZERO);

#ifdef LIST_DEBUG_MODE
    case LIST_NOT_CONSTRUCTED_USE:
      return TO_STR(LIST_NOT_CONSTRUCTED_USE);
    case LIST_DESTRUCTED_USE:
      return TO_STR(LIST_DESTRUCTED_USE);
#endif
...
void switchToIndexSearch(List* list) {
  ...
  ListNode* newNodes =
      (ListNode*)((char*)calloc(1, list->capacity * sizeof(ListNode)
...
void dumpPrintErrors(List* list, const char* indentation) {
  ...
  for (unsigned char i = 0; i < LIST_MAX_ERRORS_COUNT; i++) {
    ...
    if (currBit == 1) {
      ...
      LG_Write("ERROR 0x%X: %s", LG_STYLE_CLASS_ERROR, currBit,
               getErrorStr((ListError)currBit));
...
void dumpGraph(List* list) {
  ...
  fprintf(graphFile,
          ...
          "\tHEAD [label=\"Head\", fontsize=16.0, fontcolor=\"#F0FFFF\", "
          "fillcolor=\"#7B68EE\"];\n"
          "\tTAIL [label=\"Tail\", fontsize=16.0, fontcolor=\"#F0FFFF\", "
          "fillcolor=\"#7B68EE\"];\n"
          "\tNULL [label=\"NULL\", fontsize=16.0, fontcolor=\"#F0FFFF\", "
          "fillcolor=\"#DC143C\"];\n\n"
  ...
  for (size_t i = 1; i < list->capacity; i++) {
    ...
    if (list->nodes[i].prev != -1) {
      fprintf(graphFile, "[%lu] %lg|{pos\\n%lu|", i, list->nodes[i].value,
    ...
    if (list->nodes[i].next == 0) {
      fprintf(graphFile, "NULL");
    } else {
      fprintf(graphFile, "%d", list->nodes[i].next);
    }
    ...
    if (list->nodes[i].prev == 0) {
      ...
      fprintf(graphFile, "%d", list->nodes[i].prev);
    }
    ...
    if (list->nodes[i].prev != -1) {
      ...
      fprintf(graphFile, "}}\", fillcolor=\"#FFA07A\"];\n");
    }
  ...
  for (size_t i = 1; i < list->capacity; i++) {
    if (list->nodes[i].prev != -1 &&
        list->nodes[list->nodes[i].next].prev != -1) {
      fprintf(graphFile, "\n\tNODE%lu:<nxt> -> ", i);

      if (list->nodes[i].next == 0) {
        fprintf(graphFile, "NULL");
      } else {
        fprintf(graphFile, "NODE%d", list->nodes[i].next);
      }

      if (list->nodes[i].prev != -1) {
        fprintf(graphFile, " [constraint=false];\n");
      } else {
        fprintf(graphFile, " [color=\"#FFA07A\", constraint=false];\n\n");
      }
      ...
      if (list->nodes[i].prev != -1) {
        ...
        if (list->nodes[i].prev == 0) {
          fprintf(graphFile, "NULL [color=\"#A9A9A9\", style=\"dashed\"];\n\n");
        } else {
          fprintf(graphFile,
                  "NODE%d [color=\"#A9A9A9\", style=\"dashed\", "
                  "constraint=false];\n\n",
                  list->nodes[i].prev);
        }
      }
  ...
  if (list->head == 0) {
    ...
    fprintf(graphFile, "NODE%d", list->head);
  }
  ...
  if (list->tail == 0) {
    ...
    fprintf(graphFile, "NODE%d", list->tail);
  }
  fprintf(graphFile, " [color=\"#7B68EE\"];\n}");
...
void dump(List* list) {
  ...
  if ((list->errorStatus & LIST_NOT_CONSTRUCTED_USE) != 0 ||
      (list->errorStatus & LIST_DESTRUCTED_USE) != 0) {
    LG_Write(") [0x%X] \n", list);
  } else {
    LG_Write(
        ") [0x%X] "
